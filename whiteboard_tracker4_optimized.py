...
class WhiteboardDetector:
    def __init__(self, debug: bool = False):
        ...
        # Pre-allocate arrays for Sobel calculations
        self._sobel_x_cache = None
        self._sobel_y_cache = None
        self._sobel_mag_cache = None
        
        # Cache for color range arrays to avoid repeated array creation
        self.color_ranges = {
            'blue': (np.array([100, 30, 50], dtype=np.uint8), np.array([130, 255, 255], dtype=np.uint8)),
            'red_1': (np.array([0, 30, 50], dtype=np.uint8), np.array([10, 255, 255], dtype=np.uint8)),
            'red_2': (np.array([170, 30, 50], dtype=np.uint8), np.array([180, 255, 255], dtype=np.uint8)),
            'purple': (np.array([110, 20, 50], dtype=np.uint8), np.array([170, 255, 255], dtype=np.uint8)),
            'green': (np.array([40, 30, 50], dtype=np.uint8), np.array([80, 255, 255], dtype=np.uint8))
        }
    ...
    def find_whiteboard_surface_region(self, image: np.ndarray) -> np.ndarray:
        ...
        h, w = gray.shape
        h_half = h >> 1  # Bit shift is faster than division
        ...
        # Pre-calculate percentiles once for efficiency (vectorized)
        p60, p70, p75 = np.percentile(lower_half, [60, 70, 75])
        
        # Create mask for whiteboard surface using multiple strategies - ORIGINAL LOGIC
        masks = []
        
        # Strategy 1: Reasonably bright regions (vectorized)
        bright_thresh = max(150, p70)
        bright_mask = (lower_half > bright_thresh).astype(np.uint8) * 255
        masks.append(('bright', bright_mask))
        
        # Strategy 2: Top 40% brightest pixels (vectorized)
        top_mask = (lower_half > p60).astype(np.uint8) * 255
        ...
        for name, mask in masks:
            # Clean up mask using optimized morphological operations
            clean_mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, self.kernel_7x7, iterations=1)
            clean_mask = cv2.morphologyEx(clean_mask, cv2.MORPH_CLOSE, self.kernel_7x7, iterations=1, dst=clean_mask)
        ...
        if best_mask is None:
            ...
            best_mask = (lower_half > p75).astype(np.uint8) * 255
        ...
        if num_labels > 1:
            ...
            full_mask = (labels == largest_idx).astype(np.uint8) * 255
    ...
    def filter_structural_edges(self, edges: np.ndarray, boundary_region: np.ndarray) -> np.ndarray:
        ...
        # Apply boundary mask (in-place operation)
        boundary_edges = cv2.bitwise_and(edges, boundary_region, dst=edges.copy())
        ...
        w_025, w_075 = w * 0.25, w * 0.75
        h_025, h_075 = h * 0.25, h * 0.75
        
        # Vectorized component filtering
        areas = stats[1:, cv2.CC_STAT_AREA]
        ...
        min_dims = np.maximum(np.minimum(widths, heights), 1)
        aspect_ratios = np.maximum(widths, heights) / min_dims
        
        # Boolean masks for filtering conditions
        area_mask = areas >= 8
        aspect_mask = aspect_ratios > 1.5
        
        near_boundary_mask = (
            (xs < w_025) | (xs + widths > w_075) | 
            (ys < h_025) | (ys + heights > h_075)
        )
        
        # Combined conditions
        keep_mask = (
            area_mask & aspect_mask & 
            (near_boundary_mask | (aspect_ratios > 3) | (areas > 30))
        )
        ...
        for i in range(1, num_labels):
            if keep_mask[i-1]:
                ...
                if self.debug:
                    print(f"      Kept component: area={areas[i-1]}, aspect={aspect_ratios[i-1]:.1f}, near_boundary={near_boundary_mask[i-1]}")
            elif self.debug:
                print(f"      Rejected component: area={areas[i-1]}, aspect={aspect_ratios[i-1]:.1f}")
        ...
        # Morphological cleanup (in-place)
        cv2.morphologyEx(filtered_edges, cv2.MORPH_CLOSE, self.kernel_3x3, dst=filtered_edges)
    ...
    def detect_clean_edges(self, image: np.ndarray, boundary_region: np.ndarray) -> List[Tuple[str, np.ndarray]]:
        ...
        # Method 4: Optimized Sobel with caching
        if self._sobel_x_cache is None or self._sobel_x_cache.shape != (h, w):
            self._sobel_x_cache = np.empty((h, w), dtype=np.float64)
            self._sobel_y_cache = np.empty((h, w), dtype=np.float64)
            self._sobel_mag_cache = np.empty((h, w), dtype=np.float64)
        
        cv2.Sobel(blur5, cv2.CV_64F, 1, 0, ksize=3, dst=self._sobel_x_cache)
        cv2.Sobel(blur5, cv2.CV_64F, 0, 1, ksize=3, dst=self._sobel_y_cache)
        
        # Optimized magnitude calculation
        np.hypot(self._sobel_x_cache, self._sobel_y_cache, out=self._sobel_mag_cache)
        sobel_thresh = np.percentile(self._sobel_mag_cache, 94)
        sobel_edges = (self._sobel_mag_cache > sobel_thresh).astype(np.uint8) * 255
    ...
    def score_line_against_edges(self, rho: float, theta: float, edges: np.ndarray) -> Tuple[float, int]:
        ...
        if abs(b) > abs(a):  # More horizontal - vectorized approach
            x_range = np.arange(0, w, dtype=np.int32)
            y_values = (rho - x_range * a) / b
            valid_mask = (y_values >= 0) & (y_values < h)
            
            if np.any(valid_mask):
                valid_x = x_range[valid_mask]
                valid_y = np.round(y_values[valid_mask]).astype(np.int32)
                
                total_samples = len(valid_x)
                # Vectorized neighborhood check
                for x, y in zip(valid_x, valid_y):
                    y_start, y_end = max(0, y-1), min(h, y+2)
                    if np.any(edges[y_start:y_end, x] > 0):
                        edge_hits += 1
        else:  # More vertical - vectorized approach
            y_range = np.arange(0, h, dtype=np.int32)
            x_values = (rho - y_range * b) / a
            valid_mask = (x_values >= 0) & (x_values < w)
            
            if np.any(valid_mask):
                valid_y = y_range[valid_mask]
                valid_x = np.round(x_values[valid_mask]).astype(np.int32)
                
                total_samples = len(valid_y)
                # Vectorized neighborhood check
                for x, y in zip(valid_x, valid_y):
                    x_start, x_end = max(0, x-1), min(w, x+2)
                    if np.any(edges[y, x_start:x_end] > 0):
                        edge_hits += 1
    ...
    def deduplicate_lines(self, lines: List[Tuple[float, float, float, int, str]]) -> List[Tuple[float, float, float, int, str]]:
        ...
        # Pre-compute angles to avoid repeated calculations
        angles = [np.degrees(line[1]) for line in lines]
        ...
        for i, line in enumerate(lines):
            ...
            is_duplicate = False
            for j, existing in enumerate(unique_lines):
                existing_rho, existing_theta = existing[:2]
                existing_angle = angles[lines.index(existing) if existing in lines else np.degrees(existing_theta)]
        ...
        for method_name, edges in edge_images:
            ...
            for config in hough_configs:
                ...
                if lines is not None and len(lines) > 0:
                    ...
                    # Pre-compute angles for all lines
                    ...
                    for i, line in enumerate(lines):
                        ...
                        # Angle classification (optimized)
    ...
    def detect_markings(self, image: np.ndarray, surface_mask: np.ndarray, edge_lines: List[Tuple[float, float]] = None) -> np.ndarray:
        ...
        if edge_lines:
            ...
            for rho, theta in edge_lines:
                ...
                # Optimized line mask creation
                if abs(b) > 0.001:  # Avoid division by zero
                    x_coords = np.arange(w)
                    y_lines = (rho - x_coords * a) / b
                    valid_mask = (y_lines >= 0) & (y_lines <= h)
                    
                    for x in x_coords[valid_mask]:
                        y_line = int((rho - x * a) / b)
                        if 0 <= y_line <= h:
                            line_mask[y_line:, x] = 255
                
                # Intersect with previous line masks (area below ALL detected edges)
                cv2.bitwise_and(below_edges_mask, line_mask, dst=below_edges_mask)
        ...
        # Detect each color range using cached arrays
        for color_name, (lower_bound, upper_bound) in self.color_ranges.items():
            ...
            cv2.morphologyEx(color_mask, cv2.MORPH_OPEN, self.kernel_3x3, dst=color_mask)
            cv2.morphologyEx(color_mask, cv2.MORPH_CLOSE, self.kernel_5x5, dst=color_mask)
            ...
            if num_labels > 1:
                ...
                areas = stats[1:, cv2.CC_STAT_AREA]
                valid_components = np.where(areas >= 20)[0] + 1
                
                for i in valid_components:
                    component_mask = (labels == i).astype(np.uint8) * 255
                    cv2.bitwise_or(all_markings, component_mask, dst=all_markings)
                    
                    if self.debug:
                        print(f"    {color_name}: found component with {areas[i-1]} pixels")
        
        # Final cleanup of combined markings
        cv2.morphologyEx(all_markings, cv2.MORPH_CLOSE, self.kernel_3x3, dst=all_markings)
    ...
    def extract_marking_regions(self, marking_mask: np.ndarray) -> List[Tuple[int, int]]:
        ...
        # Find all marking pixel coordinates - optimized
        ...
        for x, y in marking_points:
            ...
            for px, py in marking_points:
                if (px, py) not in used_points:
                    ...
                    if (abs(px - x) <= box_half_size and abs(py - y) <= box_half_size):
